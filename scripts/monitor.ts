...
async function startMonitor() {
  ...
  console.log("🚀 Connecting to Somnia RPC:", SOMNIA_RPC);
    ...
    try {
      ...
      if (currentBlock > lastBlock) {
        for (let b = lastBlock + 1; b <= currentBlock; b++) {
          console.log(`📦 New block detected: ${b}`);

          const block = await provider.getBlock(b, false); // Get transaction hashes
          if (!block || !block.transactions) {
            console.warn(`⚠️ No transactions in block ${b}`);
            continue;
          }

          console.log(`🔍 Block ${b} contains ${block.transactions.length} transactions`);

          for (const txHash of block.transactions) {
            // Ensure txHash is a string
            if (typeof txHash !== "string") {
              console.warn(`⚠️ Invalid transaction hash format in block ${b}:`, txHash);
              continue;
            }
            ...
            try {
              ...
              if (!tx) {
                console.warn(`⚠️ Failed to fetch transaction ${txHash} in block ${b}`);
                continue;
              }
            } catch (txError) {
              console.warn(`⚠️ Error fetching transaction ${txHash}:`, txError);
              continue;
            }
            ...
            if (tx.to?.toLowerCase() !== CONTRACT_ADDRESS) {
              console.log(`ℹ️ Skipping tx ${txHash}: not related to contract ${CONTRACT_ADDRESS}`);
              continue;
            }

            console.log(`🔍 Tx detected for contract: ${tx.hash}`);
            ...
            try {
              ...
              console.warn(`⚠️ Failed to fetch receipt for tx ${tx.hash}:`, receiptError);
              const gasPrice = tx.gasPrice || (await provider.getGasPrice());
              gasFee = ethers.formatEther(tx.gasLimit * gasPrice) + " SOM";
            }
            ...
            // Format notification message with Tx Hash as a clickable link
            const message = `🚨 New Tx on Somnia!\n` +
                           `Tx Hash: [${tx.hash}](${EXPLORER_URL}${tx.hash})\n` +
            ...
            try {
              await notifyAll(message, {
                ...
                color: 0x3498db,
                fields: [
                  { name: "Tx Hash", value: `[${tx.hash}](${EXPLORER_URL}${tx.hash})`, inline: true },
              ...
              console.log(`✅ Notification sent for tx ${tx.hash}`);
            } catch (notifyError) {
              console.error(`❌ Failed to send notification for tx ${tx.hash}:`, notifyError);
            }
          }
        }
